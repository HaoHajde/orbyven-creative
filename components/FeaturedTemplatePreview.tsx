...
export default function FeaturedTemplatePreview({ kind }: { kind: FeaturedPreviewKind }) {
  ...
  const haoBg = {
    backgroundImage:
      "linear-gradient(90deg,rgba(0,0,0,.90),rgba(0,0,0,.55) 53%,rgba(0,0,0,.20)),url('https://images.unsplash.com/photo-1746593934498-b335e4e04845?auto=format&fit=crop&w=1200&q=68')",
  } as CSSProperties;

  return (
    <BrowserFrame url="haos-customs.ro" dark>
      <div className="min-h-[338px] bg-[#050505] text-white">
        <div className="flex items-center justify-between border-b border-white/8 bg-black/72 px-4 py-3">
          <div><p className="text-[7px] font-semibold">HAO&apos;S CUSTOMS</p><p className="text-[4px] uppercase tracking-[.17em] text-[#d9bc82]/55">premium detailing studio</p></div>
          <span className="rounded-full border border-[#d9bc82]/30 px-3 py-2 text-[5px] font-bold text-[#d9bc82]">Programează</span>
        </div>
        <div className="relative min-h-[285px] bg-cover bg-center px-5 py-7" style={haoBg}>
          <div className="relative z-10 flex min-h-[230px] max-w-[72%] flex-col justify-center">
            <div className="flex items-center gap-2"><span className="h-px w-7 bg-[#d9bc82]/65" /><p className="text-[5px] font-bold uppercase tracking-[.22em] text-[#d9bc82]">Hao&apos;s</p></div>
            <h3 className="mt-3 text-[42px] font-semibold leading-[.80] tracking-[-.078em] sm:text-[52px]">CUSTOMS</h3>
            <p className="mt-3 text-[6px] leading-3 text-white/48">Detailing profesionist. Interior. Exterior. Protecție.</p>
            <div className="mt-4 flex gap-2">
              <span className="rounded-full bg-[#d9bc82] px-3 py-2 text-[5px] font-bold text-black">Vezi serviciile</span>
              <span className="rounded-full border border-white/14 bg-black/25 px-3 py-2 text-[5px] font-semibold">Before / After</span>
            </div>
          </div>
          <div className="absolute bottom-4 right-4 h-16 w-16 rounded-full border border-[#d9bc82]/16 shadow-[0_0_55px_rgba(217,188,130,.08)]" />
        </div>
      </div>
    </BrowserFrame>
  );
