...
- Supabase production reported 19 applied migration records versus 11 SQL migration files in `main`. **Do not rename existing files, reset production, or insert fabricated migration history.** Recover exact SQL from original sources/backups and compare live definitions before selecting a new baseline for an empty test database.
