  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        console.error(error);
        if (!cancelled) {
          setErrorMessage("Nu am putut verifica starea contului. Încearcă din nou.");
          setChecking(false);
        }
      }
    };
      ...
      setErrorMessage(
        message.includes("duplicate") || message.includes("unique")
          ...
          : message.includes("already belongs")
            ? "Contul este deja asociat unei companii ORBYVEN."
            : "Compania nu a putut fi creată. Încearcă din nou."
      );
      setLoading(false);
      return;
    }
        ...
        <form onSubmit={submit} className="py-14 md:py-20">
          <section className="max-w-3xl">
            ...
            <p className="mt-6 max-w-2xl text-[15px] leading-7 text-[#6e6e73] dark:text-[#a1a1a6]">
              Spune-ne numele companiei și alege instrumentele cu care vrei să începi. Le poți modifica ulterior din workspace dacă rolul tău permite asta.
            </p>
          </section>
